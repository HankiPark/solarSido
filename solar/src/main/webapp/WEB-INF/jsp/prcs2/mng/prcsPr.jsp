...
<body>
	...
	<div class="col-3">
		<div
			...
			style="margin-left: 30px; margin-right: 30px; padding-left: 20px;padding-top:20px; margin-bottom: 30px; height:250px">
			<div class="card-body">

				<div data-role="fieldcontain">
					<label for="defandroid">지시번호</label>
					 <input name="indica"
						id="indica" type="text">
				...
				<div data-role="fieldcontain" class="row" style="margin-top:20px">
					<label class="col-6" style="padding-left:70px">시작</label><label class="col-6" style="padding-left:80px">종료</label>
					</div>
					<div class="row">
					<button type="button" id="start" class="col-5" style="height: 50px;background-color:green">
						<i class="fas fa-play"></i>
					</button>					
					<button type="button" id="end" class="col-5" style="height: 50px;margin-left:40px;background-color:red">
...
<div style="margin-left: 20px; margin-right: 30px; padding-left: 40px;padding-top:20px; margin-top: 30px; height:250px">
		<div >
		<progress value="0" max="100" id="pr5"></progress><span id="pct5"class="progText">0%</span>
		<span style="margin-left:100px">총 생산량 : </span><span id="cnt5">0</span>
	</div>
	<div class="row">
	<div class=" col-5">
		<img id="eqmImg1" class="eqmImg"
			src="${pageContext.request.contextPath}/images/eqm1.png">
			<progress value="0" max="100" id="pr1"></progress><span id="pct1"class="progSemiText">0%</span>
			<br>1번공정:
		<span id="cnt1">0</span>
	...
	<div class=" col-5">
		<img id="eqmImg2" class="eqmImg"
			...
			<progress value="0" max="100" id="pr2"></progress><span id="pct2"class="progSemiText">0%</span>
			<br>2번공정:
		<span id="cnt2">0</span>
	...
	<div class=" col-5">
		<img id="eqmImg3" class="eqmImg"
			src="${pageContext.request.contextPath}/images/eqm3.png">
				<progress value="0" max="100" id="pr3"></progress><span id="pct3"class="progSemiText">0%</span>
			<br>3번공정:
		<span id="cnt3">0</span>
	...
	<div class=" col-5">
		<img id="eqmImg4" class="eqmImg"
			...
			<progress value="0" max="100" id="pr4"></progress><span id="pct4"class="progSemiText">0%</span>
			<br>4번공정:
		<span id="cnt4">0</span>
...
$("#start").on('click',function(){
	...
	function repeat(){
		$.ajax({
			...
			$("#pr1").val(res.p2+res.p3+res.p4+res.p5);
			$("#pr2").val(res.p3+res.p4+res.p5);
			$("#pr3").val(res.p4+res.p5);
			$("#pr4").val(res.p5);
			$("#pr5").val(res.p5);

			$("#pct1").text(Math.ceil((res.p2+res.p3+res.p4+res.p5)/res.len*100)+'%');
			$("#pct2").text(Math.ceil((res.p3+res.p4+res.p5)/res.len*100)+'%');
			$("#pct3").text(Math.ceil((res.p4+res.p5)/res.len*100)+'%');
			$("#pct4").text(Math.ceil(res.p5/res.len*100)+'%');
			$("#pct5").text(Math.ceil(res.p5/res.len*100)+'%');
